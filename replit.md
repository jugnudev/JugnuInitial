...
- **Ticketing System (Stripe Connect)**: Fully integrated into Communities Settings tab with consolidated single signup flow and premium copper-charcoal design. Uses Stripe Connect Express model for direct-to-business payments. **Revenue Model**: Jugnu operates on a subscription-based model - businesses keep 100% of ticket revenue with NO platform fees or commissions. Revenue comes exclusively from monthly/yearly subscriptions. **Single Signup Flow**: Users must apply for a business account via `/business-signup`, get admin approval, then can enable ticketing. No separate organizer signup exists. Stripe Connect columns (`stripe_account_id`, `stripe_onboarding_complete`, `stripe_charges_enabled`, `stripe_payouts_enabled`, `stripe_details_submitted`) are stored directly in the `organizers` table. **Authentication**: All ticketing endpoints use `communitiesStorage.getOrganizerByUserId()` to look up approved business accounts via session userId. Legacy localStorage and header-based auth removed. **Frontend Pages**: `/tickets/organizer/dashboard` features a tabbed interface with "Events" (public-facing view) and "Manage Events" (management tools) tabs. `/tickets/organizer/events/new` fetches organizer data from `/api/tickets/organizers/me` endpoint, shows loading states, and displays "Business Account Required" message with CTA to `/business-signup` when no approved account exists. **Onboarding flow**: Checks for active organizer status, creates Stripe Connect account with slug-based return URLs (`/communities/:slug?tab=settings&ticketing=success`). **Payment flow**: Uses `on_behalf_of` and `transfer_data.destination` for direct routing to connected accounts. Webhooks handle account updates and payment fulfillment. **TicketingSettingsCard**: Displays blue "Apply for Business Account" CTA when no organizer account exists, orange "Enable Ticketing" when approved but not connected to Stripe, and green status card showing "You keep 100% of ticket revenue" message and quick actions when connected. Status refresh checks `charges_enabled` instead of requirements count to avoid infinite loading. **Image Upload**: Multer-based endpoint at `/api/tickets/events/upload-image` handles event cover images with 5MB limit and MIME validation, uploading to Supabase storage bucket `ticket-event-images`. **Event Creation**: Backend accepts nested `{event, tiers}` payload structure, creating events and ticket tiers atomically in single transaction. **My Orders**: Accessible via user profile dropdown (Receipt icon), displays purchased tickets grouped by organizer/community with premium copper-charcoal design. Backend endpoint `/api/tickets/my-tickets` returns tickets pre-grouped into upcoming/past arrays with full organizer metadata. Frontend features mobile-optimized layout with search functionality, tab navigation, and comprehensive error handling with inline retry actions. **Check-in Dashboard**: Premium QR scanner with mobile-optimized design featuring copper gradient buttons, animated camera icon, scanning frame with corner indicators, jade accent success cards, and glassmorphism effects. Manual check-in table with real-time search (name/email/serial), status filtering, and working check-in buttons. Premium copper-charcoal tab navigation with 56px+ touch targets. Audit logging is best-effort (non-blocking) to prevent check-in failures. **Email Confirmations**: Purchase confirmation emails use SendGrid via `server/tickets/email-service.ts` which sends HTML emails with QR codes, event details, and calendar links. Emails sent for both free and paid tickets after order completion. **Communities Manage Events Tab**: Premium event management interface accessible from Communities "Manage Events" tab (visible only to owners/moderators). Features large, prominent event cover photos with minimal overlays to showcase imagery, glassmorphism effects throughout, and comprehensive copper-charcoal aesthetic. Mobile layout: Vertical stack with cover photo at top (256px height), event details, vertically-stacked stats for better touch targets, and full-width h-14 (56px) action buttons. Desktop layout: Horizontal split with cover photo on left (2 columns, minimal vignette overlay), event details in middle (2 columns), and action buttons on right (1 column). All buttons meet 56px minimum height requirement. Action buttons: Copper gradient "Manage Attendees" (primary), jade gradient "Check-in", blue gradient "Analytics", purple gradient "Edit Event", and copper-outline "View Public Page". Stats displayed with copper accents, gradient text, and premium borders. Premium interactions include hover scale transforms (scale-105), color-specific shadow glows, active states (scale-95), and smooth 300ms transitions. All management actions (Manage Attendees, Check-in, Analytics, Edit Event) link to existing ticketing infrastructure at `/tickets/organizer/events/:eventId/*`. Component renders using CommunityManageEvents which fetches events via `/api/tickets/organizers/:organizerId/published-events` endpoint.
- **Communities Platform**: Offers notification preferences (email frequency, digest scheduling), growth features (invite links, member referrals, discovery), and an analytics dashboard. Includes a real-time chat with WebSocket server, online presence, and role-based permissions. Supports post reactions, media uploads (images/videos), and "Post as Business" functionality. The platform is in Free Beta, with visual badges indicating beta status and disabled billing checks. A comprehensive giveaway system is included, with automated winner selection and prize management. An in-app notification system with email delivery, frequency preferences, and a dedicated Notification Center page is also present. **Polls**: Full-featured polling system with **voter visibility** for owners/moderators on non-anonymous polls, displaying voter names, avatars, timestamps, and voting choices in a premium copper-charcoal dialog. Security enforced via role-based access control, cross-community validation, and anonymous poll privacy protection. **Navigation**: Premium mobile-optimized tabs on `/communities` index page featuring glassmorphism effects, copper gradient active states, and proper 48px touch targets for mobile usability. **Events Tab Visibility Toggle**: Community owners/moderators can control whether the Events tab is visible to regular members via a toggle in the Manage Events tab. When disabled, only owners and moderators can see ticketed events. Setting stored in `communities.show_events_tab` field (defaults to true). Premium copper-charcoal toggle UI with glassmorphism styling. NOTE: Database column `show_events_tab` must be manually added to Supabase `communities` table as `BOOLEAN NOT NULL DEFAULT true` if not already present. **View as Member Mode**: Community owners and moderators can toggle "View as Member" mode from the community header dropdown menu to see their community exactly as regular members would. When enabled, all management features (tabs, controls, admin tools) are hidden, and a sticky jade-gradient banner appears at the top with an "Exit" button. State persists per-community in localStorage using `useLayoutEffect` to prevent cross-community state leaks. Accessible via data-testid="toggle-view-as-member". **Database Count Synchronization**: Communities table maintains denormalized `total_members` and `total_posts` columns that are automatically synchronized via helper functions (`updateCommunityMemberCount`, `updateCommunityPostCount`) triggered on all membership and post CRUD operations (create/update/delete), ensuring accurate counts without N+1 query overhead on community detail pages.
