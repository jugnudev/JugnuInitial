...
- **Ticketing System (Stripe Connect)**: Fully integrated into Communities Settings tab. Uses Stripe Connect Express model for direct-to-business payments with automatic platform fee collection. Community businesses enable ticketing with one click from their Settings tab - no separate signup needed. The system auto-creates tickets_organizers records linked to existing Communities accounts via user_id. Onboarding flow includes Stripe Connect setup with slug-based return URLs (`/communities/:slug?tab=settings&ticketing=success`). Payment flow uses `on_behalf_of` and `transfer_data.destination` for direct routing to connected accounts. Webhooks handle account updates and payment fulfillment. The TicketingSettingsCard component displays orange "Enable Ticketing" CTA when not connected, and green status card showing platform fee percentage and quick actions when connected.
