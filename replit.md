...
- **Subscription & Monetization Model**: Jugnu operates on a $50/month CAD subscription model with a flat fee and ZERO commission on ticket sales. Businesses keep 100% of ticket revenue, unlike competitors who charge 5-10% per ticket. Each subscription includes 2 monthly ad placement credits (1 credit = 1 placement type for 1 day). The platform is currently in FREE BETA with full access and unlimited placement credits. Database tables `community_subscriptions`, `community_payments`, and `placement_credits_usage` track subscriptions, billing history, and credit usage. The CreditsService class handles credit checking, deduction, reset, and usage tracking. Credit routes (`/api/billing/credits/*`) provide balance, checking, and usage history endpoints. The SubscriptionDashboard component displays credit balance, usage history, and renewal dates in Community Settings > Billing tab. PricingPage component at `/pricing` showcases the subscription value proposition with competitor comparison and FAQ section. BusinessSignupPage features a value proposition banner explaining FREE BETA benefits and future pricing. **Credit Spending Feature**: Community owners can feature their events using placement credits via the "Feature Event" option in the Manage Events dropdown menu. The FeatureEventDialog component allows selecting event, placement type (Events Page Banner/Homepage Featured), date range, and displays credit cost preview. Backend API (POST /api/billing/credits/spend) validates event ownership, checks credit balance (production only), verifies no placement conflicts, creates sponsor_campaigns entry, and tracks usage in placement_credits_usage table (even in FREE BETA mode for analytics). Frontend integration includes loading state protection to prevent false "insufficient credits" errors and pre-selection of events from dropdown context.
...
A mobile-first approach is implemented using Tailwind CSS. Feature flags (`ENABLE_TICKETING`, `ENABLE_COMMUNITIES`, `FF_COALITION_LOYALTY`, `VITE_ENABLE_BILLING`) control major component visibility. **Billing Infrastructure**: Platform is in FREE BETA with `VITE_ENABLE_BILLING` disabled. When enabled, the $50/month subscription model activates with credit tracking and Stripe integration. Placement credits (2 per billing cycle) allow featured placements on homepage or events page (1 credit = 1 placement for 1 day, 2 placements = 2 credits). Credits reset monthly on billing cycle date, tracked via `creditsResetDate` in `community_subscriptions`. The `placement_credits_usage` table logs all credit usage for analytics with organizer_id, campaign_id, placements_used array, and start/end dates. Database design includes separate tables for community subscriptions, payments, billing, placement credits, giveaways, entries, and winners. All metrics are processed and stored in the Pacific timezone.
