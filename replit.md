...
- **Subscription & Monetization Model**: Jugnu operates on a $50/month CAD subscription model with a flat fee and ZERO commission on ticket sales. Businesses keep 100% of ticket revenue, unlike competitors who charge 5-10% per ticket. Each subscription includes 2 monthly ad placement credits (1 credit = 1 placement type for 1 day). Database tables `community_subscriptions`, `community_payments`, and `placement_credits_usage` track subscriptions, billing history, and credit usage. The CreditsService class handles credit checking, deduction, reset, and usage tracking with full enforcement of real credit limits. Credit routes (`/api/billing/credits/*`) provide balance, checking, and usage history endpoints. The SubscriptionDashboard component displays real credit balance, usage history, and renewal dates in Community Settings > Billing tab. PricingPage component at `/pricing` showcases the subscription value proposition with competitor comparison and FAQ section. Visual "BETA" badges remain on Communities navigation and "Coming Soon" on Loyalty for marketing branding purposes. **Credit Spending Feature**: Community owners can feature their events using placement credits via the "Feature Event" option in the Manage Events dropdown menu. The FeatureEventDialog component allows selecting event, placement type (Events Page Banner/Homepage Featured), date range, and displays credit cost preview with real balance checking. Backend API (POST /api/billing/credits/spend) validates event ownership, enforces real credit balance limits, verifies no placement conflicts, creates sponsor_campaigns entry, and tracks usage in placement_credits_usage table. Frontend integration includes loading state protection to prevent false "insufficient credits" errors and pre-selection of events from dropdown context.
