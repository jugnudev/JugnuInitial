...
- **Canada-wide Event Discovery**: Multi-area event system with location-based filtering. Events page features area filter pills (All Areas, Metro Vancouver, GTA, Greater Montreal, Calgary) with premium glassmorphism design. ICS import parser extracts "Area: [location]" from Google Calendar event descriptions and normalizes to canonical area names. Backend API (`/api/community/weekly`) supports area parameter for precise filtering. Events without area metadata are backward-compatible and display under "All Areas". Area values are normalized at import time to ensure consistency.
